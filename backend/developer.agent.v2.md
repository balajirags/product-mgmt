...
Phase 9: PUSH & PR        → Push branch + create pull request via GitHub MCP
Phase 10: JIRA UPDATE     → Add "in-review" label to story
```
...
### Step 5.1 — Compile (max 5 attempts)
```bash
...
### Step 5.2 — Static Analysis (PMD) (max 3 attempts)
```bash
...
- If still failing after **3 attempts**, STOP and report the remaining violations

### Step 5.3 — Static Analysis (SpotBugs) (max 3 attempts)
```bash
...
- If still failing after **3 attempts**, STOP and report the remaining bugs

### Step 5.4 — Run Existing Tests (Regression Check) (max 5 attempts)
```bash
...
### Step 8.1 — Pre-stage safety check
...
### Step 8.4 — Commit with conventional message
```bash
...
### Step 9.2 — Create Pull Request via GitHub MCP
...
| 5 - Verify | Compile ✅ PMD ✅ SpotBugs ✅ Existing tests ✅ (or STOP after max attempts) |
...
| 9 - Push & PR | Branch pushed, PR created (PR failure is non-blocking) |
| 10 - Jira | Label added, comment with PR link |
...
✅ Phase 9 (Push & PR): pushed → PR #42 created
✅ Phase 10 (Jira): "in-review" label added, PR linked
```
